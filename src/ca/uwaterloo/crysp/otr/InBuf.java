...
public class InBuf {
  ...
  /**
   * Initialize an OutBuf with the decoded contents of the Base64 input. The
   * input string should start with "OTR?:", followed by the base64 encoded
   * data, ends with ".".
   * 
  ...
  public int readShort() throws OTRException {
    ...
    int val = ((buf[off] & 0xff) << 8) + (buf[off + 1] & 0xff);
    off += 2;
    len -= 2;
    return val;
  }

  public void readBytes(byte[] data, int offset, int length) throws OTRException {
    if (len < length)
      throw new OTRException();
  ...
  public long readUInt() throws OTRException {
    ...
    long value = ((buf[off] & 0xff) << 24) + ((buf[off + 1] & 0xff) << 16)
