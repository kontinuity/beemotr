...
public class ConnContext implements OTRContext {
  ...
  /*
   * generation number: increment every time we go private, and never reset to 0
   * (unless we remove the context entirely)
   */
  int generation;
  ...
  public ConnContext(String aname, String rname, String prot, Provider prov, UserState us) {
  ...
  void goEncrypted(OTRCallbacks callback) throws OTRException {
    ...
    if (this.msgState.getCurState() == MsgState.ST_ENCRYPTED
        ...
        && prov.compareMPI(MPI.readMPI(new InBuf(this.our_old_dh_key.getPublicKey().serialize())),
            MPI.readMPI(new InBuf(this.auth.our_dh.getPublicKey().serialize()))) == 0
        && ((this.their_keyid > 0 && this.their_keyid == auth.their_keyid && prov.compareMPI(
            MPI.readMPI(new InBuf(this.their_y.serialize())),
            MPI.readMPI(new InBuf(this.auth.their_pub.serialize()))) == 0) ||

        (this.their_keyid > 1 && this.their_keyid - 1 == this.auth.their_keyid
            && this.their_old_y != null && prov.compareMPI(
            MPI.readMPI(new InBuf(this.their_y.serialize())),
            MPI.readMPI(new InBuf(this.auth.their_pub.serialize()))) == 0))) {
   ...
   * messages about to be sent to this routine. tlvs is an array of OTRTLVs to
   * append to the private message. It is usually correct to just pass null
   * here.
   */
    ...
    switch (msgState.getCurState()) {
      ...
      case MsgState.ST_ENCRYPTED:
        ...
        try {
          ...
          /*
           * Uh, oh. Whatever we do, *don't* send the message in the clear.
           */
  ...
  void setSmpTrust(OTRCallbacks callback, boolean trusted) {
    ...
    /*
     * Write the new info to disk, redraw the ui, and redraw the OTR buttons.
     */
    callback.writeFingerprints();
  }
  ...
  void initRespondSmp(String question, String secret, boolean initiating, OTRCallbacks callback)
    ...
    if (msgState.getCurState() != MsgState.ST_ENCRYPTED)
      return;
    /*
     * Construct the combined secret as a SHA256 hash of: Version byte (0x01),
     * Initiator fingerprint (20 bytes), responder fingerprint (20 bytes),
     * secure session id, input secret
     */
  ...
  /**
   * Initiate the Socialist Millionaires' Protocol and send a prompt question to
   * the buddy
   */
  public void initiateSmp_q(String question, String secret, OTRCallbacks callback)
      throws OTRException {
   ...
   * received messages to this routine. If no Exception is thrown, and the
   * return value is not null, replace the received message with msg in the
   * returned StringTLV, and deliver that to the user instead. If the return
   * value is null, then the message you received was an internal protocol
    ...
    switch (msgtype) {
            ...
            if (end != res.length && end + 1 != res.length) {
              ...
              if (tlv != null) {
                if (nextMsg == SM.EXPECT1) {
                  ...
                  if (this.smstate.smProgState != SM.PROG_CHEATED) {
                    callback.handleSmpEvent(OTRCallbacks.OTRL_SMPEVENT_ASK_FOR_SECRET, this, 25,
                        null);
                  } else {
                    callback.handleSmpEvent(OTRCallbacks.OTRL_SMPEVENT_CHEATED, this, 0, null);
                    this.smstate.nextExpected = SM.EXPECT1;
      ...
      case OTRMessage.MSG_NOTOTR:
        if (this.msgState.getCurState() != MsgState.ST_UNENCRYPTED
          ...
          callback.handleMsgEvent(OTRCallbacks.OTRL_MSGEVENT_RCVDMSG_UNENCRYPTED, this,
              message.toString());
        }
        ignore_message = 1;
        break;
      ...
      default:
        /*
         * We received an OTR message we didn't recognize. Ignore it, but make a
         * log entry.
         */
   ...
   * notice that we're doing so if we're currently ENCRYPTED, and we think he's
   * logged in.
   * 
   * @throws OTRException
   */
