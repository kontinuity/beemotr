...
public class Proto {
  ...
  static void revealMacs(ConnContext conn, DHSesskeys sess1, DHSesskeys sess2) {
    int numnew = sess1.rcvmacused + sess1.sendmacused + sess2.rcvmacused + sess2.sendmacused;
  ...
  public static void rotateDHKeys(ConnContext conn) throws OTRException {
    ...
    revealMacs(conn, conn.sesskeys[1][0], conn.sesskeys[1][1]);
   ...
   * @throws OTRException
   */
  public static void rotateYKeys(ConnContext conn, DHPublicKey new_y) throws OTRException {
    // Rotate the public key
    conn.their_old_y = conn.their_y;

    // Rotate the session keys
    revealMacs(conn, conn.sesskeys[0][1], conn.sesskeys[1][1]);
  ...
  public static DataMessage createData(ConnContext conn, byte[] msg, byte flags, OTRTLV[] tlvs)
    ...
    if (conn.msgState.getCurState() != MsgState.ST_ENCRYPTED || conn.their_keyid == 0) {
    ...
    byte[] msg2mac = new byte[2 + 1 + 1 + 4 + 4 + ourdh.length + 8 + 4 + enc.length];
    msg2mac[1] = 2;
    msg2mac[2] = 3;
    ...
    DataMessage dm = new DataMessage((short)2, flags, conn.our_keyid - 1, conn.their_keyid,
        ourdh_big, tophalf, new Data(enc), trimmed, new Data(conn.savedMacKeys));
  ...
  /**
   * Accept an OTR Data Message in datamsg. Decrypt it and return the plaintext
   */
  public static byte[] acceptData(ConnContext context, DataMessage msg, byte[] flags)
      throws OTRException {
    ...
    /*
     * We don't take any action on this message (especially rotating keys) until
     * we've verified the MAC on this message. To that end, we need to know
     * which keys this message is claiming to use.
     */
    ...
    DHSesskeys sess = context.sesskeys[context.our_keyid - (int)recipient_keyid][context.their_keyid
        - (int)sender_keyid];
    byte[] msg2mac = new byte[2 + 2 + 4 + 4 + their_y.length + 8 + 4 + data.length];
    msg2mac[1] = 2;
    msg2mac[2] = 3;
    ...
    System.arraycopy(data, 0, msg2mac, 12 + their_y.length + 12, data.length);
