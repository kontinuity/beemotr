...
public class AuthInfo {
  ...
  public static final byte[] DH_MODULUS = Util
  ...
  public static final byte[] DH_MODULUS_MINUS_2 = Util
      .hexStringToBytes("FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"
          + "29024E088A67CC74020BBEA63B139B22514A08798E3404DD"
          + "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245"
          + "E485B576625E7EC6F44C42E9A637ED6B0BFF5CB6F406B7ED"
  ...
  public void startAKE(KeyPair dhkp) throws OTRException {
    ...
    DHCommitMessage dhcm = new DHCommitMessage((short)0x2, new Data(encgx), new Data(hashgx));
  ...
  void createKeyMessage() throws OTRException {
    ...
    DHKeyMessage dhkm = new DHKeyMessage((short)2, MPI.readMPI(new InBuf(pub)));
  ...
  /**
   * Handle an incoming D-H Commit Message. If no error is returned, the message
   * to send will be left in lastauthmsg. Generate a fresh keypair to use.
   */
  void handleCommit(byte[] commitmsg, KeyPair dhkpa) throws OTRException {

    byte[] res = commitmsg;

    DHCommitMessage dhcm = DHCommitMessage.readDHCommitMessage(new InBuf(res, 3, res.length - 3),
        (short)2);
    // Header
    if (res.length < 3 || res[0] != 0x00 || res[1] != 0x02 || res[2] != 0x02) {
  ...
  byte[] calculatePubkeyAuth(PrivKey privkey, HMACKey mackey, AESKey enckey) throws OTRException {
    ...
    int totallen = ourpubbuf.length + theirpubbuf.length + 2 + privkey.pubkeySize() + 4;
    ...
    System.arraycopy(privkey.pubkey_data, 0, buf, lenp, privkey.pubkeySize());
    ...
    byte[] sigbuf = dsa.sign((DSAPrivateKey)privkey.getKeyPair().getPrivateKey(), macbuf);
   ...
   * given a MAC key, and encryption key. The fingerprint of the received public
   * key will get put into their_fingerprint, and the received keyid will get
   * put in their_keyid. The encrypted data pointed to by authbuf will be
   * decrypted in place.
   * 
   * @throws Exception
   */
  void checkPubkeyAuth(byte[] authbuf, HMACKey mackey, AESKey enckey) throws OTRException {
  ...
  /**
   * Handle an incoming Reveal Signature Message. If no Exception is raised, and
   * havemsgp is 1, the message to be sent will be left in lastauthmsg. Use the
   * given private authentication key to sign messages.
   * 
   * @throws OTRException
  ...
  void handleRevealsig(byte[] revealmsg, PrivKey privkey) throws OTRException {
    ...
    RevealSignatureMessage rsm = RevealSignatureMessage.readRevealSignatureMessage(new InBuf(tmp),
        (short)2);
