...
public class JCADHKeyPairGenerator extends ca.uwaterloo.crysp.otr.crypt.DHKeyPairGenerator {
  ...
  /*
   * public static void main(String[] args) { DHKeyPairGenerator kg = new
   * DHKeyPairGenerator(); if (!kg.getAlgorithm().equals("DH")) {
   * System.out.println("FAILURE: Not correct algorithm type."); } // Check that
   * keys were generated properly try { KeyPair kp = kg.generateKeyPair();
   * System.out.println("Keys generated:"); DHPublicKey pubkey =
   * (DHPublicKey)kp.getPublicKey(); DHPrivateKey privkey =
   * (DHPrivateKey)kp.getPrivateKey(); byte[] y = pubkey.getY(); byte[] x =
   * privkey.getX(); System.out.println("Private: " +
   * MPI.fromBytes(x).toString()); System.out.println("Public: " +
   * MPI.fromBytes(y).toString()); } catch (OTRCryptException e) {
   * System.out.println("FAILURE: Key generation failure.");
   * System.out.println(e.getMessage()); } catch (OTRException e) {
   * System.out.println("FAILURE: Key generation failure.");
   * System.out.println(e.getMessage()); } System.out.println(
   * "Tests complete. If there were no errors, test was successful."); }
   */
}
