...
public class JCADSAKeyPairGenerator extends ca.uwaterloo.crysp.otr.crypt.DSAKeyPairGenerator {
  ...
  public KeyPair generateKeyPair() throws OTRCryptException {
    ...
    try {
      ...
      return new KeyPair(new JCADSAPrivateKey((java.security.interfaces.DSAPrivateKey)priv),
   ...
   * JCADSAKeyPairGenerator kg = new JCADSAKeyPairGenerator(); if
   * (!kg.getAlgorithm().equals("DSA")) {
   ...
   * System.out.println("Keys generated:"); JCADSAPublicKey pubkey =
   * (JCADSAPublicKey)kp.getPublicKey(); JCADSAPrivateKey privkey =
   * (JCADSAPrivateKey)kp.getPrivateKey(); System.out.println("Public: " + new
   * java.math.BigInteger(pubkey.getY()).toString());
   * System.out.println("Private: " + new
   * java.math.BigInteger(privkey.getX()).toString()); } catch
   * (OTRCryptException e) {
