...
public class JCAAESKey extends ca.uwaterloo.crysp.otr.crypt.AESKey {
  ...
  /**
   * Constructs the wrapping instance of the given AES key using the JCA
   * provider.
   * 
  ...
  /*
   * public static void main(String[] args) { byte[] content = {(byte) 0x01,
   * (byte) 0x02, (byte) 0xff, (byte) 0xde, (byte) 0xab}; AESKey k = new
   * AESKey(content); byte[] encodedKey = k.getEncoded(); // Verify key encoding
   * & decoding for (int i = 0; i < encodedKey.length; i++) { if (content[i] !=
   * encodedKey[i]) { System.out.println("Byte mismatch."); } }
