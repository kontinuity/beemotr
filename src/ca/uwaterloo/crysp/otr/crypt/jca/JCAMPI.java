...
public class JCAMPI extends ca.uwaterloo.crysp.otr.crypt.MPI {
  ...
  public static ca.uwaterloo.crysp.otr.crypt.MPI toMPI(BigInteger val) throws OTRException {
    byte[] v = toBytes(val);
    ca.uwaterloo.crysp.otr.crypt.MPI ret = ca.uwaterloo.crysp.otr.crypt.MPI.readMPI(new InBuf(v));
    return ret;
  }
  ...
  /**
   * Convert a BigInteger to a standard byte[] format. Outputs four bytes of
   * length (big-endian), followed by that many bytes of big-endian data.
   * Leading 0 bytes are removed, and the data is always treated as unsigned.
   */
  ...
  /**
   * Converts the byte array to a BigInteger object. The first 4 bytes indicate
   * the length of the rest of the data. The length as well as the actual value
  ...
  /**
   * Read a BigInteger object from InBuf. The first 4 bytes indicate the length
   * of the rest of the data. The length as well as the actual value of the
   * BigInteger object is interpreted as big-endian.
   * 
   * @return a BigInteger containing the value as specified by input.
   * @throws OTRException
   */
