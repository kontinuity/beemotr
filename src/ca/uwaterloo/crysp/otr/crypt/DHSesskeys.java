...
public class DHSesskeys {
  ...
  /**
   * Compare two counter values (8 bytes each). Return 0 if ctr == ctr2, < 0 if
   * ctr < ctr2 (as unsigned 64-bit values), > 0 if ctr > ctr2.
   */
  ...
  public void computeSession(KeyPair our_dh, DHPublicKey their_pub) throws OTRException {
    ...
    MPI ourint = MPI.readMPI(new InBuf(((DHPublicKey)our_dh.getPublicKey()).getY()));
