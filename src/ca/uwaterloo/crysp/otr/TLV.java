...
public class TLV extends OTRTLV {
  ...
  public void serialize(byte[] buf, int offset, int length) throws OTRException {
  ...
  public byte[] serialize(OTRTLV[] tlvs) throws OTRException {
    int len = 0;
  ...
  public OTRTLV[] parse(byte[] buf, int offset, int length) throws OTRException {
   ...
   * " "); } } private void dump() { System.out.print("Type = " + type +
   * " Length = " + len + " Data = ("); dump(value); System.out.println(")"); }
   * static public void main(String[] args) { new TLV(0, null, 0, 0).dump(); new
   * TLV(0, "".getBytes()).dump(); new TLV(1, "".getBytes()).dump(); new TLV(1,
   * "a".getBytes()).dump(); TLV a = new TLV(40000, "abcdef".getBytes());
   * a.dump(); try { byte[] as = a.serialize(); dump(as);
   * System.out.println(""); } catch (OTRException e) {
   * System.out.println("OTRException"); } byte[] bs = { 5, 1, 0, 1, 6, 0, 3, 0,
   * 0, 1, 0, 0, 7, 0, 0, 0, 3, 1, 2, 3 }; try { TLV[] ba = TLV.parse(bs); for
   * (int i=0;i<ba.length;++i) ba[i].dump(); } catch (OTRException e) {
