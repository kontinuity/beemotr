 ...
 * private keys. Most clients will only need one of these. If you have separate
 * files for these things for (say) different users, use different UserStates.
 * If you've got only one user, with multiple accounts all stored together in
 * the same fingerprint store and privkey store files, use just one UserState.
 * 
...
public class UserState implements ca.uwaterloo.crysp.otr.iface.OTRInterface {
  ...
  public StringTLV messageReceiving(String accountname, String protocol, String sender,
      String message, OTRCallbacks callback) throws Exception {
    ConnContext conn = getConnContext(accountname, protocol, sender);
    return conn.messageReceiving(message, callback);
  }

  public String messageSending(String accountname, String protocol, String recipient,
      String message, OTRTLV[] tlvs, int fragPolicy, OTRCallbacks callback) throws Exception {
  ...
  /** Unit tests. */
  /*
   * public static void main(String[] args) { UserState u = new UserState(new
   * ca.uwaterloo.crysp.otr.crypt.jca.Provider()); Account a = new
   * Account("ian", "XMPP"); try { PrivKey p = u.getPrivKey(a, false);
   * System.out.println(p == null); p = u.getPrivKey(a, true); p.dump(); } catch
   * (OTRException e) { System.out.println("Error: " + e.getMessage()); } }
   */
}
