...
public class SM {
  ...
  public static final byte[] MODULUS_S = Util
      .hexStringToBytes("FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"
  ...
  public static final byte[] MODULUS_MINUS_2 = Util
      .hexStringToBytes("FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"
          + "29024E088A67CC74020BBEA63B139B22514A08798E3404DD"
          + "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245"
          + "E485B576625E7EC6F44C42E9A637ED6B0BFF5CB6F406B7ED"
  ...
  public static final byte[] ORDER_S = Util
      .hexStringToBytes("7FFFFFFFFFFFFFFFE487ED5110B4611A62633145C06E0E68"
  ...
  /**
   * This method accepts an array of MPIs, and returns a byte array containing a
  ...
  /**
   * Takes a byte array containing serialized and concatenated MPIs and converts
   * it to an array of MPIs. The buffer is assumed to consist of a 4-byte int
   * containing the number of MPIs in the array, followed by {size, data} pairs
   * for each MPI.
   * 
   * @throws OTRException
   */
  ...
  /**
   * Check that an MPI is in the right range to be a (non-unit) group element
   */
  public static boolean checkGroupElem(MPI g, Provider prov) {
    byte[] two = {
      2
    };
    if (prov.compareMPI(g, new MPI(two)) < 0 || prov.compareMPI(g, new MPI(SM.MODULUS_MINUS_2)) > 0) {
      return true;
    }
    return false;
  }
  ...
  public static boolean checkExpon(MPI x, Provider prov) {
    ...
    if (prov.compareMPI(x, new MPI(one)) < 0 || prov.compareMPI(x, new MPI(SM.ORDER_S)) >= 0) {
      return true;
    }
    return false;
  }
  ...
  /**
   * Verify a proof of knowledge of a discrete logarithm. Checks that c = h(g^d
   * x^c)
   * 
   * @throws OTRException
   */
  public static int checkKnowLog(MPI c, MPI d, MPI g, MPI x, int version, Provider prov)
      throws OTRException {
   ...
   * @throws OTRException
   */
  public static MPI[] proofEqualCoords(SMState state, MPI r, int version, Provider prov)
      throws OTRException {
   ...
   * @throws OTRException
   */
  public static int checkEqualCoords(MPI c, MPI d1, MPI d2, MPI p, MPI q, SMState state,
      int version, Provider prov) throws OTRException {

    /*
     * To verify, we test that hash(g3^d1 * p^c, g1^d1 * g2^d2 * q^c) = c If
     * indeed c = hash(g3^r1, g1^r1 g2^r2), d1 = r1 - r*c, d2 = r2 - secret*c.
     * And if indeed p = g3^r, q = g1^r * g2^secret Then we should have that:
     * hash(g3^d1 * p^c, g1^d1 * g2^d2 * q^c) = hash(g3^(r1 - r*c + r*c), g1^(r1
     * - r*c + q*c) * g2^(r2 - secret*c + secret*c)) = hash(g3^r1, g1^r1 g2^r2)
     * = c
     */
   ...
   * @throws OTRException
   */
  public static int checkEqualLogs(MPI c, MPI d, MPI r, SMState state, int version, Provider prov)
      throws OTRException {

    /*
     * Here, we recall the exponents used to create g3. If we have previously
     * seen g3o = g1^x where x is unknown during the DH exchange to produce g3,
     * then we may proceed with: To verify, we test that hash(g1^d * g3o^c,
     * qab^d * r^c) = c If indeed c = hash(g1^r1, qab^r1), d = r1- x * c And if
     * indeed r = qab^x Then we should have that: hash(g1^d * g3o^c, qab^d r^c)
     * = hash(g1^(r1 - x*c + x*c), qab^(r1 - x*c + x*c)) = hash(g1^r1, qab^r1) =
     * c
     */
   ...
   * MPI array whose elements correspond to the following: [0] = g2a, Alice's
   * half of DH exchange to determine g2 [1] = c2, [2] = d2, Alice's ZK proof of
   * knowledge of g2a exponent [3] = g3a, Alice's half of DH exchange to
   * determine g3 [4] = c3, [5] = d3, Alice's ZK proof of knowledge of g3a
   * exponent
   * 
   * @throws OTRException
   */
  ...
  public static void step2a(SMState bstate, byte[] input, int received_question, Provider prov)
    ...
    if (checkGroupElem(msg1[0], prov) || checkExpon(msg1[2], prov) || checkGroupElem(msg1[3], prov)
   ...
   * [2] = d2, Bob's ZK proof of knowledge of g2b exponent [3] = g3b, Bob's half
   * of DH exchange to determine g3 [4] = c3, [5] = d3, Bob's ZK proof of
   ...
   * otrl_sm_step2b. Output is a serialized mpi array whose elements correspond
   * to the following: [0] = pa, [1] = qa, Alice's halves of the (Pa/Pb) and
   * (Qa/Qb) values [2] = cp, [3] = d5, [4] = d6, Alice's ZK proof that pa, qa
   * formed correctly [5] = ra, calculated as (Qa/Qb)^x3 where x3 is the
   * exponent used in g3a [6] = cr, [7] = d7, Alice's ZK proof that ra is formed
   * correctly
   * 
   * @throws OTRException
   */
  public static byte[] step3(SMState astate, byte[] input, Provider prov) throws OTRException {
    ...
    if (checkGroupElem(msg2[0], prov) || checkGroupElem(msg2[3], prov)
        ...
        || checkExpon(msg2[2], prov) || checkExpon(msg2[5], prov) || checkExpon(msg2[9], prov)
   ...
   * correctly This method also checks if Alice and Bob's secrets were the same.
   * If so, it returns NO_ERROR. If the secrets differ, an INV_VALUE error is
   * returned instead.
   * 
   * @throws OTRException
   */
  public static byte[] step4(SMState bstate, byte[] input, Provider prov) throws OTRException {
    ...
    if (checkGroupElem(msg3[0], prov) || checkGroupElem(msg3[1], prov)
        || checkGroupElem(msg3[5], prov) || checkExpon(msg3[3], prov) || checkExpon(msg3[4], prov)
  ...
  /*
   * public static void main(String[] args) throws OTRException{ Provider prov =
   * new ca.uwaterloo.crysp.otr.crypt.jca.JCAProvider(); String a = "0f"; String
   * b = "02"; String mod = "05"; MPI res = prov.subm(new MPI(SM.MODULUS_S), new
   * MPI(SM.MODULUS_MINUS_2), new MPI(SM.MODULUS_S)); String ss =
   * Util.bytesToHexString(res.getValue()); System.out.println(ss); }
   */
