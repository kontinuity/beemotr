...
public class PrivacyList extends ListActivity {
  ...
  protected Dialog onCreateDialog(int id) {
    ...
    switch (id) {
      case DIALOG_CREATE:
        ...
        break;
      case DIALOG_DELETE:
        dialog = new DeletePrivacyList(PrivacyList.this, mPrivacyListManager, mCurrPrivacyListName)
            .create();
        dialog.setOnDismissListener(new OnDismissListener() {
  ...
  private class PrivacyListListener extends IPrivacyListListener.Stub {
    ...
    @Override
    public void setPrivacyList(String listName, List<PrivacyListItem> listItem)
    ...
    public void updatedPrivacyList(final String listName) throws RemoteException {
      ...
      mHandler.post(new Runnable() {
        ...
        public void run() {
          try {
            mPrivacyListNames.clear();
            // Not that much lists and require some server queries to know if
            // the list has been
            // updated/deleted or set to default/active by this activity or
            // another IM client.
            mPrivacyListNames.addAll(mPrivacyListManager.getPrivacyLists());
          } catch (RemoteException e) {
            Log.e(TAG, e.getMessage());
