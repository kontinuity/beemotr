...
public class ContactList extends Activity {
  ...
  /**
   * Map containing a list of the different contacts of a given group. Each list
   * is a @{link SortedList} so there is no need to sort it again.
   */
    ...
    if (mSelectedContact != null) {
      switch (item.getItemId()) {
        ...
        case R.id.contact_list_context_menu_userinfo_subscription:
          Dialog subscription = new ResendSubscription(ContactList.this, mXmppFacade,
  ...
  private class BeemRosterListener extends IBeemRosterListener.Stub {
        ...
        if (visible) {
          for (String group : groups) {
            if (!mListGroup.contains(group)) {
              ...
              List<Contact> tmplist = new SortedList<Contact>(new LinkedList<Contact>(),
                  mComparator);
    ...
    private void updateCurrentList(String listName, final Contact contact) {
      ...
      final boolean add = ((!hideDisconnected || Status.statusOnline(contact.getStatus())) && // must
                                                                                              // show
                                                                                              // and
     ...
     * 
     * @throws RemoteException if an error occur when communicating with the
     *           service
     */
  ...
  private class BeemContactList extends BaseAdapter implements Filterable {
    ...
    private void bindView(View view, Contact curContact) {
      if (curContact != null) {
        ...
        LevelListDrawable mStatusDrawable = (LevelListDrawable)getResources().getDrawable(
