...
public class BeemChatManager extends IChatManager.Stub {
  ...
  private class ChatListener extends IMessageListener.Stub implements ChatManagerListener {
    ...
    public void chatCreated(Chat chat, boolean locally) {
      ...
      Log.d(
          TAG,
          "Chat" + chat.toString() + " created locally " + locally + "with "
              + chat.getParticipant());
      try {
    ...
    /**
     * Create the PendingIntent to launch our activity if the user select this
     * chat notification.
     * 
    ...
    private void notifyNewChat(IChat chat) {
      ...
      try {
        CharSequence tickerText = mService.getBind().getRoster()
        ...
        notification.setLatestEventInfo(mService, tickerText,
            mService.getString(R.string.BeemChatManagerNewMessage), makeChatIntent(chat));
