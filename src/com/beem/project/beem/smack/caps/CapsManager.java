...
/**
 * Capabilities manager to implements XEP-0115. The DiscoverInfo are cached in
 * memory.
 */
public class CapsManager {
  ...
  /**
   * Set the node attribute to send in your capability. This is usually an uri
  ...
  /**
   * Load a persistent DiscoverInfo. The default implementation does nothing and
   * always return null.
   * 
  ...
  /**
   * Store a DiscoverInfo for persistence. The default implementation does
   * nothing.
   * 
   * @param ver the ver hash of the DiscoverInfo
  ...
  /**
   * Check if the discover info correspondig to the ver hash is in cache. This
   * implementation checks the memory cache. If the info is not in cache it is
   * necessary to request it from the network.
   * 
  ...
  private DiscoverInfo getOwnInformation() {
    ...
    DiscoverInfo.Identity id = new DiscoverInfo.Identity("client",
  ...
  private void initSupportedAlgorithm() {
    ...
    String[] algo = new String[] {
        "sha-1", "md2", "md5", "sha-224", "sha-256", "sha-384", "sha-512"
    };
    for (String a : algo) {
      try {
